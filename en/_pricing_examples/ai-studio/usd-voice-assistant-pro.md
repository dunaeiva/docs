{% calc [currency=USD] 4 × {{ sku|USD|foundation_models.text_generation.v1|number }} %} × 2 + {{ sku|USD|foundation_models.realtime_audio.input.v1|string }} × 60 + {{ sku|USD|foundation_models.realtime_audio.output.v1|string }} × 20 = {% calc [currency=USD] 8 × {{ sku|USD|foundation_models.text_generation.v1|number }} %} + {% calc [currency=USD] {{ sku|USD|foundation_models.realtime_audio.input.v1|number }} × 60 %} + {% calc [currency=USD] {{ sku|USD|foundation_models.realtime_audio.output.v1|number }} × 20 %}

Total: {% calc [currency=USD] 8 × {{ sku|USD|foundation_models.text_generation.v1|number }} + {{ sku|USD|foundation_models.realtime_audio.input.v1|number }} × 60 + {{ sku|USD|foundation_models.realtime_audio.output.v1|number }} × 20 %}.
