...
- Visual Studio Code {#vscode}
  ...
  1. Install the [{{ ca-full-name }}](https://sourcecraft.dev/portal/code-assistant/) or [Roo Code](https://roocode.com/) extension.
  1. Configure the extension:

     {% cut "{{ ca-full-name }}" %}

     1. Click **SourceCraft Code Assistant** in the program's left-hand panel.
     1. Click ![chevron-up](../../_assets/console-icons/chevron-up.svg) **Select API configuration** at the bottom of the chat window and click **{{ yandex-cloud }} {{ ai-studio-name }}**.
     1. In the settings window that opens, make sure the `OpenAI Compatible` option is selected in the **API provider** field and the **Base URL** field is set to `https://{{ api-host-llm }}/v1`, and close the settings window.
     ...
     1. In the **Model** field, specify the model URI in `gpt://<folder_ID>/<model_ID>/latest` format, where:
         ...
         * `<model_ID>`: [Model ID]({{ link-docs-ai }}ai-studio/concepts/generation/models), e.g., `qwen3-235b-a22b-fp8` or `gpt-oss-120b`.
         ...
         The selected model will be used only for the assistant. The {{ ca-full-name }} autocompletion feature will continue using the default model.
