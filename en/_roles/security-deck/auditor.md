The `security-deck.auditor` role enables viewing info on {{ dspm-name }}, {{ cspm-name }}, and {{ kspm-name }} resources, on alerts and alert sinks, as well as on scan jobs and the number of detected security threats. With this role, you cannot view masked and unprocessed data.
...
* View info on the [{{ kspm-name }} settings](../../security-deck/concepts/kspm.md) and operations, as well as the list of exceptions from rules.
