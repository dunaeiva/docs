...
* View info on the [{{ kspm-name }}](../../security-deck/concepts/kspm.md) settings, Managed Service for Kubernetes [clusters](../../managed-kubernetes/concepts/index.md#kubernetes-cluster) connected to {{ kspm-name }}, exceptions from rules, exceptions from the control scope, {{ kspm-name }} users, and {{ kspm-name }} operations.
