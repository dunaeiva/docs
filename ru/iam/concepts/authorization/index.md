...
* [IAM-токен](iam-token.md) — рекомендуемый и наиболее безопасный тип. Подходит для большинства операций, например для [создания виртуальной машины](../../../compute/operations/vm-create/create-linux-vm.md). Не работает в отдельных сервисах или API, где предполагается использование других типов учетных данных.
    ...
    Можно настроить автоматический перевыпуск IAM-токенов с помощью [refresh-токенов](./refresh-token.md) для пользователей с любым типом [аккаунта](../users/accounts.md). Это позволит им работать в [{{ yandex-cloud }} CLI](../../../cli/index.yaml) без необходимости повторно аутентифицироваться в браузере по истечении срока жизни IAM-токена.
