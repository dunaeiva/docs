...
Объем использованных ресурсов определяется как количество и размер [ресурсных единиц](concepts/application-load-balancer.md#lcu-scaling) — внутренних виртуальных машин, создаваемых для работы балансировщика в каждой его зоне доступности.

{% note info %}

Возможность изменения размера [ресурсных единиц] включается отдельно. Для включения необходимо обратиться к аккаунт-менеджеру. Подтверждение запроса на включение возможности может занимать до 14 дней и не гарантируется. Услуга включается с даты получения подтверждения. 
